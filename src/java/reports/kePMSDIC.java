 ...
 */
public class kePMSDIC extends HttpServlet {
HttpSession session;
String partner_name,age,gender,county,partner_id,dic_name;
int achieved;
String startDate,endDate,month,partnername,clientid,agebracket;
...
// rw4.createCell(1).setCellValue("Number");
 ...
 cell4.setCellValue("ACHIEVED");
      ...
      String getData="SELECT count(personal_information.client_id),dic.dic_name,"
            + "CASE " +
...
"END AS SEX,personal_information.completionyear,personal_information.completionmonth"
              ...
              + "LEFT JOIN dic ON dic.dic_id=personal_information.dic_id"
      ...
      while(conn.rs.next()){
          dic_name=age=gender=county="";achieved=0;
...
response.setHeader("Content-Disposition", "attachment; filename=PWP_kePMS_REPORT_FOR_PEPFAR_YEAR_"+pepfaryear+"_PER_DIC_FOR_"+partner_name+"_CREATED_ON_"+CRT.timestamp()+".xlsm");
...
pkg.close();  
  ...
  response.sendRedirect("kePMS.jsp");     
    ...
    try {
        ...
        Logger.getLogger(kePMSDIC.class.getName()).log(Level.SEVERE, null, ex);
