...
String group_status,provider_status,dic_id;
String existingGroups;
String cfullname,cfname,cmname,clname,ward_id;
  ...
  client_id=IG.current_id();
     ...
     if(group_status.equals("no")){
      ...
      linked_groupid=group_id;
       if(has_group.equals("no")){ group_id="0";}
       System.out.println("group exist===================="+group_id);
      }
      else{
     ...
     if(provider_status.equals("no")){
      String checkprovider="SELECT provider_id,group_ids FROM service_provider WHERE (fname=? && lname=?) || (national_id=? && national_id!=? && national_id!=?)";
      ...
      if(conn.rs.next()==true){
          ...
          System.out.println("Provider_exist===================="+provider_id);
      }
      ...
      String checkIfRegistered="SELECT client_id FROM personal_information WHERE (national_id=? && national_id!=?) || (ccc_no=?)";
...
+ "hiv_year,art_status,hf_id,ccc_no,registration_date,approved_by,designation,approval_date,timestamp,dic_id,ward_id,linked_group)"
+ " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
          ...
          conn.pst.setString(12, employment_status);
          conn.pst.setString(13, education_level);
          conn.pst.setString(14, under_18);
          conn.pst.setString(15, ovc_children);
          conn.pst.setString(16, group_id);
          conn.pst.setString(17, provider_id);
          conn.pst.setString(18, partner_id);
          conn.pst.setString(19, hiv_year);
          conn.pst.setString(20, art_status);
          conn.pst.setString(21, hf_id);
          conn.pst.setString(22, ccc_no);
          conn.pst.setString(23, registration_date);
          conn.pst.setString(24, approved_by);
          conn.pst.setString(25, designation);
          conn.pst.setString(26, approval_date);
          conn.pst.setString(27, timestamp);
          conn.pst.setString(28, dic_id);
          conn.pst.setString(29, ward_id);
          conn.pst.setString(30, linked_groupid);
          conn.pst.executeUpdate();
          
        fullname=fname+" "+mname+" "+lname;   
          
     status="<font color=\"green\">"+fullname+" was added successfully to the system.</font>" ;    
          ...
          status="<font color=\"red\">"+fullname+" was not added to the system.</font>" ;
      }
      
      
   session.setAttribute("addedClient", status);
...
if(conn.conn!=null){conn.conn.close();}
   response.sendRedirect("ClientRegistrationForm.jsp");
    }
