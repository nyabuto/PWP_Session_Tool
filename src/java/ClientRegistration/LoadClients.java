...
int pos,data2;
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
          ...
          session.setAttribute("partnerID", partner_id);
          session.setAttribute("districtID", district_id);
          pos=0;
         data="<thead>"
                  + "<tr>"
                  + "<th>NO</th>"
          ...
          String getClients="SELECT client_id,personal_information.fname,personal_information.mname,personal_information.lname,group_id,"
           ...
           if(dob.equals("")){
            dobStatus="(<font color=\"red\">Un-edited</font>)"   ;
           }
           else{
//           dobStatus="(<font color=\"blue\">edited</font>"   ; 
         ...
         pos++;
          data+=""
                      ...
                      + "<td>"+ccc_no+"</td>"
                      + "<td>"+status+"</td>"
                      + "<td><a class=\"linkstyle\" style=\"width:60px;\" href=\"LoadEditClient?client_id="+client_id+"\" alt=\"link\">Edit</a></td>";
                  if(session.getAttribute("level").toString().equals("1") || session.getAttribute("level").toString().equals("3")){    
                  data+= "<td><a class=\"linkstyle\" style=\"width:60px;\" href=\"#\" onclick=\"return deleter("+client_id+");\">Delete</a></td>";
                          }
                          data+="</tr>"
                      + "";
...
//           
//           data2=0;
//       String getClients2="SELECT count(personal_information.client_id) FROM personal_information WHERE dob=''";
//       conn.rs=conn.st.executeQuery(getClients2);
//       if(conn.rs.next()==true){
//           data2=conn.rs.getInt(1);
//       }
//       
//       if(data2>0){
//        session.setAttribute("UnEdited", "<b><font color=\"red\">Un-edited<br>("+data2+")</font></b>");
//       } 
