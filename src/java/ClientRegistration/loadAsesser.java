 ...
 */
public class loadAsesser extends HttpServlet {
HttpSession session;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        session=request.getSession();
         ...
         data="<thead>"
          ...
          String getClients="SELECT client_id,personal_information.fname,personal_information.mname,personal_information.lname,group_id,"
           ...
           while(conn.rs.next()){
               found=0;
               System.out.println("client_id : "+conn.rs.getString(1));
               String checkAssessed="SELECT id FROM prevention_messages WHERE client_id='"+conn.rs.getString(1)+"'";
              System.out.println(checkAssessed);
               conn.rs1=conn.st1.executeQuery(checkAssessed);
               if(conn.rs1.next()==true){
                   found=conn.rs1.getString(1).length();
               }
              System.out.println("found data : "+found);
           ...
           if(found>0){
            dobStatus="(<font color=\"red\">Assessed</font>)"   ;
           }
           else{
//           dobStatus="(<font color=\"blue\">edited</font>"   ; 
         ...
         pos++;
          data+=""
                      ...
                      + "<td>"+groupname+"</td>"
                      + "<td>"+provider+"</td>"
                      + "<td>"+fullname+""+dobStatus+"</td>"
                      + "<td>"+lessonsAttended+"</td>"
                      + "<td>"+ccc_no+"</td>";
          if(found>0){
                      data+= "<td><a class=\"linkstyle\" style=\"width:100px; color: blue\" href=\"loadEditAssessment?client_id="+client_id+"\" alt=\"link\">Edit form</a></td>";
          }else{
                      data+= "<td><a class=\"linkstyle\" style=\"width:60px;\" href=\"getBasicDetails?dt="+client_id+"&&client_id=234564\" alt=\"link\">Assess</a></td>";
          }
                      data+="</tr>"
                      + "";
...
if(conn.conn!=null){conn.conn.close();}
 ...
 out.println(data);
    ...
    try {
        ...
        Logger.getLogger(loadAsesser.class.getName()).log(Level.SEVERE, null, ex);
    }
    }
